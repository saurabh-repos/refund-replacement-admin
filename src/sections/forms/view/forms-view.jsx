...
export default function FormPage() {
  ...
  useEffect(() => {
    getData();
  }, [debouncedSearch, page, rowsPerPage, region, status, refund, startDate, endDate]);
  ...
  const headLabel = [
    ...
    { id: 'initiaterId', label: 'Email address', minWidth: 130 },
    ...
    { id: 'purchasedFrom', label: 'Customer Purchased / Ordered From', minWidth: 180 },
    ...
    {
      id: 'complainHistory',
      label: 'Previous Complaint No & Details of activity (Repeat Failure Case Only)',
      minWidth: 200,
    },
    { id: 'reason', label: 'Reason for Return, Refund or Replacement', minWidth: 200 },
    {
      id: 'detailsOfParts',
                  ...
                  {dataFiltered &&
                    dataFiltered.map((row) => (
                      <FormTableRow
                        ...
                        status={row?.status}
                        pendingWith={row?.requestDetails?.assignedEmail || 'N/A'}
                        pendingDays={
                          row?.requestDetails?.assignedEmail ? calculatePendingDays(row) : 'N/A'
                        }
